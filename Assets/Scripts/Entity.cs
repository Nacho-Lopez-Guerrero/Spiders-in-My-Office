	...
	public virtual void TakeDamage(float dmg)				//virtual to allow override
	{
		...
		if(health <= 0 && !_dead)
		{
			...
			if(!_isPlayer)
			{
				...
				Instantiate(deathSplashBlood, new Vector3(transform.position.x, transform.position.y + 0.5f, transform.position.z), transform.rotation);
